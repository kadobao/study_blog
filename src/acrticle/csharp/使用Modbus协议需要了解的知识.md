...
只有需要 **拆分到多个寄存器** 的值，才需要处理高低位。

> **核心原则**：16位能装下的值（一个寄存器），就不需要处理高低位；装不下的（不管是 int、float、double），都必须拆分处理。

| 数据类型 | 位数 | 占用寄存器数 | 是否需要处理高低位 |
|:--------:|:----:|:----------:|:-----------------:|
| ushort / short | 16位 | 1个 | ❌ 不需要 |
| int / uint | 32位 | 2个 | ✅ 需要 |
| float | 32位 | 2个 | ✅ 需要 |
| long / ulong | 64位 | 4个 | ✅ 需要 |
| double | 64位 | 4个 | ✅ 需要 |
...
> **判断原则**：16位能装下的值，就不需要处理高低位；装不下的（不管是 int、float、double），都必须拆分处理。
