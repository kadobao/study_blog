...
| 绑定方式 | JS层 | 模板层 | 用途 |
|---------|------|--------|------|
| 文本插值 | `ref()` 定义数据 | `{{ 变量 }}` | 显示数据 |
| 属性绑定 | `ref()` 定义状态 | `:属性="变量"` | 动态属性 |
| 事件绑定 | 定义处理函数 | `@click="函数"` | 用户交互 |
| 双向绑定 | `ref()` 定义数据 | `v-model="变量"` | 表单同步 |
| 条件渲染 | `ref()` 定义状态 | `v-if="变量"` | 条件显示 |
| 列表渲染 | `ref()` 定义数组 | `v-for="item in 数组"` | 遍历渲染 |
| 计算属性 | `computed(()=>{})` | `{{ 计算属性 }}` | 派生数据 |
| 侦听器 | `watch(()=>{})` | 无需模板 | 响应变化 |
