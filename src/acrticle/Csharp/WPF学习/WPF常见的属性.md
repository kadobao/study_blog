...
- **属性名**：`Margin`
- **说明**：用于设置控件与其他控件之间的外边距。`Margin`是一个`Thickness`结构，可以分别设置上下左右的外边距，例如`Margin="10,20,30,40"`表示上边距为10，右边距为20，下边距为30，左边距为40。
- **示例**:
```xaml
...
- **属性名**：`Padding`
- **说明**：用于设置控件内部内容与控件边框之间的内边距。`Padding`也是一个`Thickness`结构，用法与`Margin`类似，例如`Padding="5,10"`表示上下内边距为5，左右内边距为10。
- **示例**:
```xaml
