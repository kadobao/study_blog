...
**作用**：数据分组统计
- GROUP BY 的分组依据是：指定列（或表达式）的值是否相同。 值完全相同的行会被归入同一个组，每组最终在结果中输出为一行。
- 相同的列值归为一组，会产生多个组
...
按 `Department` 列的值是否相同进行分组，相同的行归入同一个组：

- **技术部组**：Department='技术部' 的有张三、李四 2 行 → 折叠成 1 行，`COUNT(*)` 数这组里的行数 = **2**
- **市场部组**：Department='市场部' 的有王五、赵六 2 行 → 折叠成 1 行，`COUNT(*)` = **2**
- **人事部组**：Department='人事部' 的只有孙七 1 行 → 折叠成 1 行，`COUNT(*)` = **1**

5 行原始数据 → 3 个组 → 每组输出 1 行，所以结果只有 3 行。
...
同样按 `Department` 分组，但这次每组内做的是**累加 Salary**：

- **技术部组**：组内是张三(8000)、李四(9000) → `SUM(Salary)` = 8000 + 9000 = **17000**
- **市场部组**：组内是王五(6000)、赵六(7000) → 6000 + 7000 = **13000**
- **人事部组**：组内只有孙七(5000) → **5000**
