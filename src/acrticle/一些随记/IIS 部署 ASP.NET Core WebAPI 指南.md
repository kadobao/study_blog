...
6. **选择所需功能（推荐勾选以下）**
   - 在"功能"页面，建议勾选：
     - .NET Framework 4.8（或你项目需要的版本）
     - ASP.NET 4.8（如果你部署的是 .NET Framework 的 Web API 或 MVC）
     - 如果是 .NET Core / .NET 5+，不需要勾选 ASP.NET，但需后续安装 .NET Hosting Bundle

7. **确认并安装**
   - 点击"安装"，等待完成（可能需要几分钟）
   - 安装完成后，可选：重启服务器（通常不需要）

### ✅ 第二步：验证 IIS 是否安装成功

打开浏览器，访问 `http://localhost`
如果看到 IIS 欢迎页面，说明安装成功。